...
class Bomb:
    def __init__(self, window_size_x, window_size_y, FPS, speed, acceleration, radius, color_period, color_1, color_2):
        self.time = 0
        self.FPS = FPS
        self.speed = speed
        self.acceleration = acceleration
        ...
        self.radius = radius
        self.y_pos = 0
        self.x_pos = randint(0+self.radius, self.window_size_x - self.radius)
        self.color_period = color_period
    ...
    def start(self):
        ...
        self.y_pos += self.speed*100/self.FPS
        if(self.time % self.color_period == 0):
            if(self.color == self.color_1):
                self.color = self.color_2
            else:
                self.color = self.color_1
        if(self.y_pos >= self.window_size_y):
            self.speed += self.acceleration
            self.y_pos = 0
            self.x_pos = randint(0+self.radius, self.window_size_x - self.radius)
        return int(self.x_pos), int(self.y_pos), self.color
